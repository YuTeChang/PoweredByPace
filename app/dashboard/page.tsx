...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // Ensure sessions and groups are loaded (lazy load on dashboard page)
        // Only call if we're actually on the dashboard page
        if (typeof window !== "undefined" && window.location.pathname === '/dashboard') {
        ...
        console.warn('[Dashboard] Failed to load groups:', error);
      }
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-8">
      <div className="max-w-md w-full space-y-6 sm:space-y-8 text-center">
        ...
        {/* Empty State */}
        {isLoaded && groups.length === 0 && standaloneSessions.length === 0 && !session && (
        ...
        {/* Action Buttons */}
        {isLoaded && (groups.length > 0 || standaloneSessions.length > 0) && (
